package com.clubecerto.apps.app.adapter;
...
import com.clubecerto.apps.app.activities.OffersActivity;
...
public class StoreOfferAdapter {
    ...
    private View prepareView(View layout, Offer offer) {
        ...
        detail.setText(Html.fromHtml(offer.getShort_description()));


        if (offer.getCurrency() != null) {

            if (offer.getValue_type() != null && !offer.getValue_type().equals("")) {
                if (offer.getValue_type().equalsIgnoreCase("Percent") && (offer.getOffer_value() > 0 || offer.getOffer_value() < 0)) {
                    DecimalFormat decimalFormat = new DecimalFormat("#0");
                    price.setText(decimalFormat.format(offer.getOffer_value()) + "%");
                } else {
                    if (offer.getValue_type().equalsIgnoreCase("Price") && offer.getOffer_value() != 0) {
        ...
        } else if (offer.getValue_type().equalsIgnoreCase("unspecifie")) {
            price.setText(context.getString(R.string.promo));
        }
