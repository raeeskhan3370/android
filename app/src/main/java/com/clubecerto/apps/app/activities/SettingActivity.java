...
import com.clubecerto.apps.app.BuildConfig;
...
//        );
        ...
        Preference app_about_us = findPreference("app_about_us");

        app_about_us.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
...
//        );
    ...
    private void manageModulesVisibility() {
        ...
        if (!SettingsController.isModuleEnabled("event")) {
            mCategory.removePreference(findPreference("notif_nearby_events"));
            mCategory.removePreference(findPreference("notif_upcomingevent"));
        }

        if (!SettingsController.isModuleEnabled("messenger")) {
            mCategory.removePreference(findPreference("notif_messenger"));
        }

        if (!SettingsController.isModuleEnabled("store")) {
            mCategory.removePreference(findPreference("notif_nearby_stores"));
        }

        if (!SettingsController.isModuleEnabled("offer")) {
            mCategory.removePreference(findPreference("notif_offers"));
        }
